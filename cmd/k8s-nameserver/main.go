...
// k8s-operator to allow to resolve magicDNS names of Tailscale services in a
// Kubernetes cluster.
...
func main() {
	...
	go func() {
		...
		for {
			select {
			// TODO (irbekrm): it appears that we get a whole bunch
			// of different events (except for the WRITE one that
			// fsnotify recommends watching) on an update. We should
			// batch updates together to avoid updating resolver
			// config multiple times for each configmap update.
